...
io.on("connection", (socket) => {
	...
	socket.on("ROOM_INFO", (code, cb) => {
		...
		// console.log(room);
		if (
			(room === null || room === void 0 ? void 0 : room.ownerID) ==
			((_a =
				decodedToken === null || decodedToken === void 0
					? void 0
					: decodedToken.id) !== null && _a !== void 0
				? _a
				: "")
		) {
			cb(room);
	...
	socket.on("OWNER_REMOVE_MEMBER", (code, memberID) => {
		...
		if (!room || !decodedToken || room.ownerID !== decodedToken.id) {
			return;
		}
...
app.get("/rooms/exit", (req, res) => {
	...
	if (decodedToken && room) {
		const index = room.members.findIndex(
			(member) => member.id === decodedToken.id
		);
		if (index >= 0) {
			room.members.splice(index, 1);
		}
